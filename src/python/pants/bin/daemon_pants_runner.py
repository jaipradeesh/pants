...
# This probably shouldn't live _here_.
class MaybeShutdownSocket(object):
  def __init__(self, socket, lock, is_shutdown):
    self.socket = socket
    self.lock = lock
...
class DaemonPantsRunner(ProcessManager):
  ...
  def create(cls, sock, args, env, services, scheduler_service):
    maybe_shutdown_sock = MaybeShutdownSocket(sock, threading.Lock(), False)
    ...
    try:
      ...
      # threads.
      with cls.nailgunned_stdio(maybe_shutdown_sock, env, handle_stdin=False):
