...
@rule(TestResult, [PythonTestsAdaptor, TransitiveHydratedTarget, PyTest])
def run_python_test(test_target, transitive_hydrated_target, pytest):
  ...
  all_targets = [test_target] + [dep.root.adaptor for dep in transitive_hydrated_target.dependencies]
  ...
  for maybe_source_target in all_targets:
    if hasattr(maybe_source_target, 'sources'):
      sources_snapshot = maybe_source_target.sources.snapshot
